...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> fetchDataFromFirestore() async {
    try {
      ...
      querySnapshot.docs.forEach((doc) {
        // Check if the document contains the 'gender' field
        if (doc.data().containsKey('gender')) {
          gender = doc['gender'];
          fetchedDataList.add(gender);
          print('Gender: $gender');
        } else {
          print('Warning: Field "gender" does not exist in document ${doc.id}');
          fetchedDataList.add('Unknown');
        }
      });
                          ...
                          Row(
                            children: [
                              ClipRRect(
                                ...
                                child:Image(
                                  image:AssetImage(_getImageAssetPath(gender)),
                                  fit: BoxFit.cover,
                                  height: 70.0,
                                  width: 70.0,),
                              ),),
  ...
  String _getImageAssetPath(String gender) {
